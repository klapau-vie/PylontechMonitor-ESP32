...
## ✨ Features & Neuerungen
* **ioBroker & HA ready:** Sendet MQTT-Werte. Überträgt Auto-Discovery-Daten, sodass der Objektbaum vollautomatisch und fehlerfrei aufgebaut wird.
...
* **ElegantOTA Integration (NEU):** Firmware-Updates können nun bequem direkt über das Web-Dashboard (OTA Update Button) hochgeladen werden. Ideal für ESP32 in entfernten Subnetzen oder hinter VPNs.
...
5. Öffne die **`PylontechMonitoring.h`** und trage deine WLAN- und MQTT-Daten ein. Passe ggf. `MAX_PYLON_BATTERIES` an dein Setup an.
...
* **ElegantOTA Integration (NEW):** Firmware updates can now be uploaded directly via the web dashboard. Perfect for ESP32 devices in remote subnets or behind VPNs.
...
5. Open **`PylontechMonitoring.h`** and insert your WiFi and MQTT credentials. Adjust `MAX_PYLON_BATTERIES` if necessary.
